...
from linchemin.cheminfo.template import Pattern, PatternConstructor, Template, TemplateConstructor, \
...
def test_template_hashing():
    ...
    for k, v in reactions.items():
        if template := template_constructor.build_from_reaction_string(reaction_string=v.get('smiles'), inp_fmt='smiles'):
            h = template.hash_map
        else:
            h = None
        results[k] = h
