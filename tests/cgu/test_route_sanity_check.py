...
def test_cycle_checker():
    ...
    # if the fix_issues option is set to False, an error is raised if the route contains cycles
    with pytest.raises(CyclicRouteError) as e:
        route_checker(
            route,
            "cycle_check",
            fix_issue=False,
        )
        assert "CyclicRouteError" in str(e.type)
    checked_route = route_checker(route, "cycle_check", fix_issue=True)
