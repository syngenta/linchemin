...
def test_nx_translation(ibm1_path, az_path):
    ...
    graph = json.loads(open(az_path).read())
    nx_graph_bp = translator('az_retro', graph[1], 'networkx', out_data_model='bipartite')
    edge_labels = [data['label'] for data in nx_graph_bp.in_edges.values()]
    assert edge_labels.count('REACTANT') == 4
    assert edge_labels.count('REAGENT') == 0
    assert edge_labels.count('PRODUCT') == 2
