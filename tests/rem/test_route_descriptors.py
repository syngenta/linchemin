...
def test_longest_sequence():
    ...
    mp_syngraph = translator('az_retro', graph[3], 'syngraph', out_data_model='monopartite_reactions')
...
def test_subset():
    ...
    g2 = translator('az_retro', graph[0], 'syngraph', out_data_model='monopartite_reactions')
    # A non-subset is correctly identified as such
    assert not is_subset(g2, mp_syngraphs)


def test_find_duplicates():
    graph1 = json.loads(open("../test_file/ibmrxn_retro_output_raw.json").read())
    ibm_routes = [translator('ibm_retro', g, 'syngraph', out_data_model='bipartite') for g in graph1]
    graph2 = json.loads(open("../test_file/az_retro_output_raw.json").read())
...
def test_get_node_consensus():
    ...
    az_routes_mp = [translator('az_retro', g, 'syngraph', out_data_model='monopartite_reactions') for g in graph2]
