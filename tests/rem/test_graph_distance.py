...
def test_similarity_factory():
    ...
    graph_az = json.loads(open("../cgu/data/az_retro_output_raw.json").read())
    ...
    ged_params = graph_distance_factory(mp_syngraphs[0], mp_syngraphs[3], ged_method='nx_ged',
    ...
    ged_matrix = graph_distance_factory(mp_syngraphs[0], mp_syngraphs[3], ged_method='nx_ged_matrix')
...
def test_compute_nodes_fingerprints():
    ...
    graph_ibm = json.loads(open("../cgu/data/ibmrxn_retro_output_raw.json").read())
    ...
    mp_reaction_fp, mp_mol_fp = compute_nodes_fingerprints(mp_syngraphs[1], reaction_fingerprints='structure_fp',
                                                           molecular_fingerprint='rdkit')
...
def test_build_similarity_matrix():
    ...
    reaction_fp1, molecule_fp1 = compute_nodes_fingerprints(mp_syngraphs[1], reaction_fingerprints='structure_fp',
                                                            molecular_fingerprint='rdkit')
    reaction_fp2, molecule_fp2 = compute_nodes_fingerprints(mp_syngraphs[2], reaction_fingerprints='structure_fp',
                                                            molecular_fingerprint='rdkit')
...
def test_similarity_matrix():
    ...
    ged = graph_distance_factory(mp_syngraphs[0], mp_syngraphs[3], ged_method='nx_ged_matrix')
    assert ged != 0.0
...
def test_compute_distance_matrix():
    ...
    routes = json.loads(open("../cgu/data/ibmrxn_retro_output_raw.json").read())
    ...
    matrix_parallelization = compute_distance_matrix(mp_syngraphs, ged_method='nx_ged',
                                                     parallelization=True, n_cpu=8)
    diag_p = pd.Series([matrix_parallelization.iat[n, n] for n in range(len(matrix_parallelization))],
...
def test_optimized_ged():
    ...
    opt_ged = nx.optimize_graph_edit_distance(nx_graphs[0], nx_graphs[1], node_match=lambda x, y: x == y)
    for g in opt_ged:
        min_g = g
...
def test_optimized_ged_workflow():
    ...
    mp_syngraphs = [translator('az_retro', g, 'syngraph', out_data_model='monopartite_reactions') for g in graph_az]
    ged_mp = graph_distance_factory(mp_syngraphs[0], mp_syngraphs[3], ged_method='nx_optimized_ged')
    ged_mp_std = graph_distance_factory(mp_syngraphs[0], mp_syngraphs[3], ged_method='nx_ged')
    assert ged_mp == ged_mp_std


def test_get_available_ged():
    assert type(get_available_ged_algorithms()) == dict and 'nx_ged' in get_available_ged_algorithms()


def test_get_default():
    assert type(get_ged_default_parameters()) == dict and 'reaction_similarity_name' in get_ged_default_parameters()


def test_ged_params():
    assert type(get_ged_parameters()) == dict and 'reaction_fp_params' in get_ged_default_parameters()
