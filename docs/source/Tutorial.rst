...
argument. By default, a "mapping pipeline" is used, in which all the available mapping tools
are called one after the other to make sure that all the reactions in all the routes are mapped.
As an alternative, you can pass a string to the ``mapper`` argument, specifying the name of your preferred tool.
...
.. warning::
    ...
    specify the ``mapper`` argument with 'rxnmapper', which is freely available.
...
.. code-block:: python
    ...
    output = process_routes({'ibmrxn_file.json': 'ibmrxn'}, # path of input file generated by IBMRXN
                             ...
                             mapper='rxnmapper'             # the preferred tool is selected
...
:func:`~linchemin.cheminfo.atom_mapping.perform_atom_mapping`  and
the :func:`~linchemin.cheminfo.atom_mapping.pipeline_atom_mapping` functions. You can check them out if
