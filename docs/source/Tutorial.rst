...
If you generated the SynGraph representation of a route predicted by a CASP tools, it is very likely that
the roles of the Molecules in the involved reactions are incorrect, with reagents appearing as reactants.
In order to improved the chemical information in your route you might want to perform the atom mapping on the
involved reactions and LinChemIn has a full machinery to do that!


Be aware, however, that the atom-to-atom mapping tools are part of our separate library ``LinChemIn_Services``
...
Extracting chemical reaction strings from routes
-------------------------------------------------

The first step for performing the atom mapping is to extract the reaction strings
from the route (although you can also prepare a set of reaction strings to be mapped independently from a route).
This can be achieved with the
:func:`~linchemin.cgu.syngraph.extract_reactions_from_syngraph` function, which takes a
SynGraph object as input (of any type) and returns a list of dictionaries, one for each reaction.

.. code-block:: python

    from linchemin.cgu.syngraph import extract_reactions_from_syngraph
    # A list of dictionaries is returned
    reactions = extract_reactions_from_syngraph(syngraph)

The dictionaries in the ``reactions`` list are in the form [{'query_id': n, 'input_string': reaction}],
which is compatible with the direct use of the atom mapping machinery implemented in LinChemIn.

Note that a list of dictionaries of very similar format con also be used as input for
creating a SynGraph object: you just need to substitute 'input_string' with 'output_string'.


Mapping chemical reaction strings
----------------------------------

In order to map one or more reaction strings, you first need to have them in the appropriate format, which is
a list of dictionaries (one dictionary for each reaction) in the form [{'query_id': n, 'input_string': reaction}].
You can get it from a SynGraph object as shown in the previous example, or you can create one yourself.
Once you have this object you can proceed with the atom mapping and you can do it in two ways:

- calling a single mapper
- calling the entire pipeline

In both cases, the output will contain a list of dictionaries in the form [{'query_id': n, 'output_string': reaction}],
that can be directly used to create a new instance of a SynGraph object.
...
If you decide to use a single mapper, you should rely on the
...
To discover which mappers are available, you can call the
:func:`~linchemin.cheminfo.atom_mapping.get_available_mappers` function.
...
If you decide to use the full mapping pipeline, you should use the
