            ...
            for reac, connections in mp_syngraph.graph.items():
                ...
                if reac not in [
                    ...
                    unmapped_smiles = cif.rdrxn_to_string(
                        reac.rdrxn, out_fmt="smiles", use_atom_mapping=False
                    )
                    prop = {
                ...
                for c in connections:
                    if c.smiles not in [
                        ...
                        unmapped_smiles = cif.rdrxn_to_string(
                            c.rdrxn, out_fmt="smiles", use_atom_mapping=False
                        )
                        prop = {
                            "node_unmapped_smiles": unmapped_smiles,
    ...
    def from_iron(self, route_iron: Iron) -> None:
        ...
        try:
            ...
            for id_n, node in route_iron.nodes.items():
                ...
                dot_graph.add_node(
                    pydot.Node(
                        node.properties["node_unmapped_smiles"],
                        image=f"{id_n}.png",
                        label="",
                    )
                )
...
def az_dict_to_iron(
    route: dict, properties: dict, iron: Union[Iron, None], parent: Union[int, None]
) -> Iron:
