...
class TranslateFacade(Facade):
    ...
    def perform_functionality(self, input_format: str, input_list: list,
                              ...
                              n_cpu=DEFAULT_FACADE['translate']['value']['n_cpu']) -> tuple:
        """
    ...
    def get_available_options(self) -> dict:
        ...
        """
        return {'input_list': {'name_or_flags': ['-input_list'],
                               'default': None,
                               'required': True,
                               'type': list,
                               'choices': None,
                               ...
                               'dest': 'input_list'},
                'input_format': {'name_or_flags': ['-input_format'],
                                 'default': None,
                                 'required': True,
                                 'type': str,
                                 ...
                                 'dest': 'casp'},
                'out_format': {'name_or_flags': ['-out_format'],
...
class RoutesDescriptorsFacade(Facade):
    ...
    def perform_functionality(self, routes: list,
                              descriptors=DEFAULT_FACADE['routes_descriptors']['value']['descriptors']) -> tuple:
        """
...
class GedFacade(Facade):
    ...
    def perform_functionality(self, routes: list,
                              ...
                              n_cpu=DEFAULT_FACADE['distance_matrix']['value']['n_cpu']) -> tuple:
        """
        ...
        try:
            dist_matrix = compute_distance_matrix(checked_routes, ged_method=ged_method, ged_params=ged_params,
                                                  parallelization=parallelization, n_cpu=n_cpu)
    ...
    def get_available_options(self) -> dict:
        ...
        return {'routes': {'name_or_flags': ['-routes'],
                ...
                'ged_params': {'name_or_flags': ['-ged_params'],
                               ...
                               'dest': 'ged_params'},
                'parallelization': {'name_or_flags': ['-parallelization'],
                                    'default': DEFAULT_FACADE['distance_matrix']['value']['parallelization'],
                                    'required': False,
                                    'type': bool,
                                    'choices': [True, False],
                                    'help': 'Whether parallelization should be used',
...
class ClusteringFacade:
    ...
    def perform_functionality(self, routes: list,
        ...
        try:
            ...
            results = clusterer(checked_routes, ged_method=ged_method, clustering_method=clustering_method,
                                ged_params=ged_params, save_dist_matrix=save_dist_matrix,
                                parallelization=parallelization,
                                n_cpu=n_cpu, **kwargs)
            meta = {'graph_type': 'monopartite' if type(routes[0]) == MonopartiteReacSynGraph else 'bipartite',
    ...
    def get_available_options(self) -> dict:
        ...
        return {'routes': {'name_or_flags': ['-routes'],
                ...
                'ged_params': {'name_or_flags': ['-ged_params'],
                               'default': DEFAULT_FACADE['distance_matrix']['value']['ged_params'],
                               'required': False, 'type': dict, 'choices': get_ged_parameters(),
                               'help': 'Parameters of the molecular and/or reaction chemical similarity',
                               'dest': 'ged_params'},
                'save_dist_matrix': {'name_or_flags': ['-save_dist_matrix'],
                ...
                'compute_metrics': {'name_or_flags': ['-compute_metrics'],
...
class MergingFacade(Facade):
    ...
    def perform_functionality(self, routes: list, out_data_model=DEFAULT_FACADE['merging']['value']['out_data_model']):
        """
...
class FacadeFactory:
    functionalities = {'translate': {'value': TranslateFacade,
                                     'info': TranslateFacade.info},
                       'routes_descriptors': {'value': RoutesDescriptorsFacade,
                                              'info': RoutesDescriptorsFacade.info},
                       'distance_matrix': {'value': GedFacade,
                                           'info': GedFacade.info},
                       'clustering': {'value': ClusteringFacade,
                                      'info': ClusteringFacade.info},
                       'subsets': {'value': SubsetsFacade,
                                   'info': SubsetsFacade.info},
                       'duplicates': {'value': DuplicatesFacade,
                                      'info': DuplicatesFacade.info},
                       'merging': {'value': MergingFacade,
...
def facade_helper(functionality=None, verbose=False):
    ...
    if functionality is None:
        av_functionalities = {f: additional_info['info'] for f, additional_info in
                              FacadeFactory.functionalities.items()}
        if verbose:
